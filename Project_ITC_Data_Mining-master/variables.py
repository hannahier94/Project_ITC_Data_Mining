...
class Variables:
    ...
    def __get_siteid(self):
        res = re.findall(f'\W*(data-fullname="\w+\")',
                         self.post.prettify())[utils['MAGIC_ZERO']]
        return res.replace(f'data-fullname="', "").replace('"', "")

    def __get_title(self):
        title = self.post.find('div', class_='entry unvoted').div. \
                    p.text.split('(')[utils['MAGIC_ZERO']].replace("'", "") \
                    [utils['MAGIC_ZERO']:utils['MAGIC_TITLE']]
        printable = set(string.printable)
        return ''.join(filter(lambda x: x in printable, title))

    def __get_titlescore(self):
        title_score = self.post.find('div', class_='entry unvoted') \
                        .div.p.text.split('(')[utils['MAGIC_ZERO']]
        pol_score = sia.polarity_scores(title_score)['compound']
        pol_score = utils['MAGIC_ONE'] if pol_score > self.POL_LIM \
            else -utils['MAGIC_ONE'] if pol_score < -self.POL_LIM \
            else utils['MAGIC_ZERO']
        return pol_score
    ...
    def __get_comments(self):
        return int(self.post.find('a', class_='comments').text.split() \
                   [utils['MAGIC_ZERO']].replace("'", ""))

    def __get_likes(self):
        return self.post.find('div', attrs={'class': 'score likes'}) \
                    .text.replace("'", "")

    def __get_dislikes(self):
        return self.post.find('div', attrs={'class': 'score dislikes'})\
                .text.replace("'", "")

    def __get_date(self):
        return re.findall(r'\d+-\d+-\w+:\d+:\d+\+\d+',
                          self.post.find('p', attrs={'class': 'tagline'})
                          .time.prettify())[utils['MAGIC_ZERO']]
    ...
    def __get_thread(self):
        return self.post.find('div', class_='entry unvoted').div.p \
                            .text.split('(')[utils['MAGIC_ONE']]\
                            .replace(')', "").replace("'", "")
    ...
    def __get_extra_bool(self, kind):
        try:
            res = re.findall(f'\W*({kind}="\w+\")',
                             self.post.prettify())[utils['MAGIC_ZERO']]
            return res.replace(f'{kind}="', "").replace('"', "") \
                       .capitalize() == 'True'
        except:
            return False
    ...
    def __get_extra_num(self, kind):
        try:
            res = re.findall(f'\W*({kind}="\w+\")',
                             self.post.prettify())[utils
                                ['MAGIC_ZERO']].replace(f'{kind}="', "") \
                                .replace('"', "")
            return int(res)
        except:
    ...
    def __get_postype(self):
        try:
            post_type = re.findall('\W*(collapsed hide-when-pinned \S+\")',
                                   self.post.prettify())[utils['MAGIC_ZERO']].\
    ...
    def __get_author_info(self):
        return self.post.find('span', class_='flair flair-seniorflair')\
                    .text.replace("'", "")
    ...
    def __set_dtypes(self):
        ...
        self.BOOL_DICT = {'spoilers': self.__get_extra_bool(kind=
                                                            'data-spoiler'),
                          'promoted': self.__get_extra_bool(kind=
                                                            'data-promoted'),
                          'crossposts': self.__get_extra_num(kind=
                                                             'data-num'
                                                             '-crossposts')}

    def parse_info(self, logger):

        """ Function to parse all datatype variables
        with relevant try/except clause"""
        ...
        for key, func in self.UNKNOWN_DICT.items():
            try:
                ...
                logger.info("{}\nCould not find {}. Appending Unknown." \
                            .format(err, key))
        ...
        for key, func in self.BOOL_DICT.items():
            try:
                ...
                logger.info("{}\nCould not find {}. Appending False." \
                            .format(err, key))
        ...
        for key, func in self.ZERO_DICT.items():
            try:
                ...
                logger.info("{} \nCould not find {}. Appending {}." \
                            .format(err, key, utils['MAGIC_ZERO']))
