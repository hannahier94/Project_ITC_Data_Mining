...
class TableUpdater(DbBuild):
    ...
    def __init__(self, res, logger):
        ...
        if not isinstance(res, dict):  # Confirm object is a dict
            res = eval(res.replace(str(False).lower(), str(False))
                       .replace(str(True).lower(), str(True)))
    ...
    def __format_insert(self, table_name, cols):
        ...
        statement = utils['INSERT_FORMATTER'].format(table_name.lower(),
                                                     cols,
                                                     params)
    ...
    def __create_inserts(self):
        ...
        for sqlid in self.res['IDS'].keys():
            self.__insert_dict[sqlid] = self.__format_insert(sqlid,
                                                             utils['TABLES']
                                                             [sqlid])

        for table in [k for k in self.res.keys() if k != 'IDS']:
            self.__insert_dict[table] = self.__format_insert(table,
                                                             self.res[table]
                                                             .keys())
    ...
    def __update_cat(self):
        ...
        for table in self.res['IDS'].keys():
            final_ids[table] = {val[utils['MAGIC_ZERO']]:
                                    val[utils['MAGIC_ONE']]
        ...
        for table in self.res['IDS'].keys():
            ...
            while i < len(self.res['IDS'][table]):
                ...
                try:
                    ...
                    self.logger.debug('{} : Could not update id for {}'
                                      .format(err, self.res['IDS'][table][i]))
                i += utils['MAGIC_ONE']
        ...
        for table, id_column in NON_ID_TABLES.items():
            ...
            while i < len(self.res[table][id_column]):
                ...
                vals = [self.res[table][col][i]
                        if self.res[table][col][i] != REDDIT_NULL
    ...
    def __update_apitable(self, list_data):
        ...
        while i < len(data[id_col]):
            ...
            try:
                ...
                self.logger.info('{} : Could not update api for {}'
                                 .format(err, data[id_col][i]))
            i += utils['MAGIC_ONE']
