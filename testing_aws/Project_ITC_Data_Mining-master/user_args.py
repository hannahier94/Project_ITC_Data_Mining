...
class UserInputs:
    ...
    def customize(self):
        """This function returns a uniform format for the
        custom input topics"""
        return self.topics
...
def parse_args():
    ...
    parser.add_argument('command', choices=['default', 'custom', 'add'],
                        type=str, nargs=utils['MAGIC_ONE'], help='''Any two 
                        word topics  should be a single word 
                        (ex: to search 'data science' and 'ML', 
                        input : --topics=datascience --topics=ML).
                        Options include 
                        default, add, custom .\n
    ...
    parser.add_argument("-s", "--sleep", type=int,
                        choices=[utils['MAGIC_ZERO'], utils['MAGIC_ONE'],
                        utils['MAGIC_TWO']], default=utils['MAGIC_ONE'],
                        help="Choose seconds to sleep, default = {} "
                        "(recommended)".format(utils['MAGIC_ONE']))

    parser.add_argument("-c", "--console", default='False',
                        choices=['False', 'DEBUG', 'INFO', 'WARNING', 'ERROR'],
                        action='store',
                        help="Set logging level for console logs, "
                        "default is set to 'False'. "
                        "\nChoices = ['False', 'DEBUG',"
                        "'INFO','WARNING','ERROR'] ")

    parser.add_argument('--topics', action='append',
                        help="Please specify each topic individually with "
