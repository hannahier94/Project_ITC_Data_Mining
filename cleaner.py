...
class CleanDicts:
    ...
    def __init__(self, logger):
        ...
        self.cols = {x.rsplit(utils['SEPERATOR'])[utils['MAGIC_ZERO']]: list() for x in self.ids}  # create a
        # dict to store cache
        ...
        for key, value in self.id_dicts.items():
            if len(value) > 0:
                for column in self.cols.keys():
                    self.cols[column].extend([x[1] for x in value if column.upper() in key])
                for a, b in value:
    ...
    def __update_id_cache(self, col_details):
        ...
        for user_info in self.mess:
            ...
            col_cache.extend([p for p in user_info[col_name] if p not in col_cache])
    ...
    def organize_info(self, mess):
        ...
        self.__sql_finalids = {table: [(v, k) for k, v in self.__sql_id_dict[table].items()]
